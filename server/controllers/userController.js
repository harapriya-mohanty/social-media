...
export const unfollowUser = async (req, res) => {
    try {
        ...
        const user = await User.findById(userId)
        user.following = user.following.filter(id => id.toString() !== _id);
        await user.save();

        const toUser = await User.findById(_id);
        toUser.followers = toUser.followers.filter(id => id.toString() !== userId);
        await toUser.save();
...
export const sendConnectionRequest = async (req, res) => {
    try {
        ...
        const connection = await Connection.findOne({$or: [
            {rom_user_id: userId, to_user_id: _id},
            {rom_user_id: _id, to_user_id: userId},
        ]
        });
        if (!connection) {
            ...
            return res.json({success: true, message: "Connection request already sent"});
        }
...
export const getUserConnections = async (req, res) => {
    try {
        ...
        const pendingConnections = (await Connection.find({to_user_id: userId, status: 'pending'}).map(connection=>connection.from_user_id));
...
export const acceptConnectionRequest = async (req, res) => {
    try {
        ...
        const connection = await Connection.findOne({from_user_id: _id, to_user_id: userId});
