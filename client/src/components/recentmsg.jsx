                    ...
                    >
                        ...
                        <div className='w-full'>
                            <div className="flex justify-between">
                                <p className="font-medium"> {message?.from_user_id?.full_name ?? message?.from_user_id?.username ?? 'Unknown'}</p>
                                <p className='text-[10px] text-slate-400'>{moment(message.createdAt).fromNow()}</p>
                            </div>
                            <div className='flex justify-between'>
                                <p className="text-gray-500">{message?.text || (message?.message_type === 'image' ? 'Sent an image' : '')}</p>
