...
const PostCard = ({ post }) => {
    ...
    const handleLike = () => {
      ...
      setLikes((prev) => {
        ...
        return safePrev.includes(id) ? safePrev.filter((x) => x !== id) : [id, ...safePrev]
      })
    }
        ...
        <div className='grid grid-cols-2 gap-2'>
            {(post?.image_urls ?? []).map((img, index) => (
              <img
                src={img}
                key={index}
                className={'w-full h-48 object-cover rounded-lg ' + ((post?.image_urls?.length ?? 0) === 1 ? 'col-span-2 h-auto' : '')}
                alt=''
              />
            ))}
        </div>
