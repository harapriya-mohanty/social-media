...
const Connections = () => {
  ...
  const dataArray = [{label:'Followers',value: followers, icon: Users}, {label:'Following',value: following, icon: UserCheck}, {label:'Connections',value: connections, icon: UserPlus}, {label:'Pending Connections',value: pendingConnections, icon: UserRoundPen}]
    ...
    <div className='min-h-screen bg-slate-50'>
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='mb-8 flex flex-wrap gap-6'>
          {dataArray.map((item,index) => (<div key={index} className='flex flex-col items-center justify-center gap-1 border h-20 w-40 border-gray-200 bg-white shadow rounded-md'><b>{item.value.length}</b><p className='text-slate-600'>{item.label}</p></div>))}
        </div>
        {/* Tabs */}
          ...
          {dataArray.map((tab) => (
            <button
              ...
              className={
                ...
                (currentTab === tab.label ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-50')
              }
            >
